        ...
        with tf.Session() as sess:
            ...
            while ts_step * batch_size < ts_size:
                loss_value_ts, acc_ts, attn_coefs = sess.run([loss, accuracy, attn_coefs],
                                                             feed_dict={
                                                                 ftr_in: features[ts_step * batch_size:(
                                                                                                                   ts_step + 1) * batch_size],
                                                                 bias_in: biases[ts_step * batch_size:(
                                                                                                                  ts_step + 1) * batch_size],
                                                                 lbl_in: y_test[ts_step * batch_size:(
                                                                                                                 ts_step + 1) * batch_size],
                                                                 msk_in: test_mask[ts_step * batch_size:(
                                                                                                                    ts_step + 1) * batch_size],
                                                                 is_train: False,
                                                                 attn_drop: 0.0, ffd_drop: 0.0})
...
if __name__ == '__main__':
    ...
    attn_discrepancy = defaultdict(list)
    for i in range(100):
        attn_coefs = main()
        for h, attn in enumerate(attn_coefs):
            attn = th.tensor(attn).squeeze(0)
            max_discrep = 0
            for k in range(attn.size(0)):
                attn_v = attn[k]
                attn_v = attn_v[attn_v.nonzero()]
                discrep = (attn_v.max() - attn_v.min()).item()
                if max_discrep < discrep:
                    max_discrep = discrep
            attn_discrepancy[h].append(max_discrep)
    for h in range(len(attn_coefs)):
        print('Head {:d}, mean of max discrep {:.4f}, '
              'std of max discrep '
              '{:.4f}'.format(h+1,
                              np.mean(attn_discrepancy[h]),
                              np.std(attn_discrepancy[h])))
